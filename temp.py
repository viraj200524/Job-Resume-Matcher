...
tabs = st.tabs(["Upload Resume", "Resume Analysis", "Candidate Matches", "All Candidates", "Email Settings"])
...
# Tab 4: All Candidates (renamed from All Jobs)
with tabs[3]:
    st.header("All Available Candidates")
    ...
    if not candidates_df.empty:
        st.subheader(f"Found {len(candidates_df)} Candidates")
        
        # Display candidate listings
        for i, candidate in candidates_df.iterrows():
            with st.expander(f"{candidate['name']} - {candidate['email']}"):
                ...
                with col1:
                    ...
                    if candidate['linkedin'] != 'None':
                        st.markdown(f"**LinkedIn:** {candidate['linkedin']}")
                    
                    st.markdown(f"**Skills:** {candidate['skills']}")
                    st.markdown(f"**Qualifications:** {candidate['qualifications']}")
                    st.markdown(f"**Experience:** {candidate['experience']}")
                    
                    if candidate['projects'] != 'None':
                        st.markdown(f"**Projects & Domains:** {candidate['projects']}")
                
                with col2:
                    # View matches button
                    if st.button("View Job Matches", key=f"view_matches_{candidate['candidate_id']}"):
                        st.session_state['user_id'] = candidate['candidate_id']
                        st.session_state['user_email'] = candidate['email']
                        
                        # Load candidate data as parsed_data
                        parsed_data = {
                            "Name": candidate['name'],
                        ...
                        st.session_state['current_tab'] = "Candidate Matches"
                        st.rerun()
    else:
        st.info("No candidates found. Try adjusting your search filters or upload some resumes.")
